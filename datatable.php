...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	{
		$sOrder = "ORDER BY  ";
		for ( $i=0 ; $i<floatval( $_GET['iSortingCols'] ) ; $i++ )
		{
			if ( $_GET[ 'bSortable_'.floatval($_GET['iSortCol_'.$i]) ] == "true" )
			{
				$sOrder .= $aColumns[ floatval( $_GET['iSortCol_'.$i] ) ]."
